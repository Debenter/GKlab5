...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener {
    ...
    private void draw(GL2 gl2) {
        ...
        if (objectNumber == 1) {
            CreateCorkscrew(gl2, 1, 12); // stworzenie korkociągu
        } else if (objectNumber == 2) {
            CreatePiramid(gl2, 1, 12); //stworzenie Piramidy
        }

    }

    public void CreateCorkscrew(GL2 gl2, int radius, int numbersOfTurns) {
        gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);

        int numVertices = numbersOfTurns * 1000;
        double angleIncrement = numbersOfTurns * Math.PI / numVertices;
        float height = -8;
    ...
    public void CreatePiramid(GL2 gl2, double size, int n) {
        ...
        for (int i = 0; i < X.length; i++) {
            gl2.glBegin(GL2.GL_POLYGON);
            gl2.glColor3f(1,1,1); // Nadanie koloru punktowi
